...
> تنويه: يمكنك ايقاف تسجيلات مكتبة `salla` فقط عن طريقها
...
store = Salla(token="TOKEN") # الان التسجيل فعال افتراضيا

# هكذا يتم ايقافه
store = Salla(token="TOKEN", enable_logging=False)
...
عند الانتهاء من مهمة ضع اكس داخل القوسين
- [x] () ... 
